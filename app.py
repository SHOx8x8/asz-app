...
# --- 【ASZ 占術×心理学：プロダクト用解明エンジン】 ---
class ASZOmniscientEngine:
    def __init__(self):
        # 心理学的キーワードに基づいた10天体の定義
        self.planets = {
            "太陽": {"role": "社会的な顔（ペルソナ）", "help": "社会で見せる自分と、獲得すべき自己像を解明します。"},
            "月": {"role": "無意識の心（本能的な安心）", "help": "リラックスした時の素の自分と、心の安定条件を示します。"},
            ...
            "土星": {"role": "規律と課題（超自我）", "help": "苦手意識があるけれど、克服すれば最大の武器になるポイントです。"},
    ...
    def get_analysis(self, y, m, d, gender):
        ...
        for i, (name, info) in enumerate(self.planets.items()):
            ...
            # 性別に応じた心理学的アプローチ
            if gender == "男性":
                detail = f"{sign}の資質を「社会的な武器」として戦略的に活用するのが鍵。外的な達成が自信に繋がります。"
            elif gender == "女性":
                detail = f"{sign}のエネルギーを「内面の調和」として感じ取って。共感が本来の輝きを引き出します。"
            else:
                detail = f"{sign}という枠に囚われず、純粋な「心理機能」としてこの力を使いこなすのが全知への道です。"
...
if submit:
    ...
    for i, data in enumerate(analysis):
        with cols[i % 2]:
            ...
            """, unsafe_allow_html=True)
    st.markdown("---")
    st.info("これがプロダクトとしての『解明結果』だ。誰に渡しても、納得感を与えられる質に仕上げたよ。💀💖")
else:
