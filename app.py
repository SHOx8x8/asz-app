...
class ASZOmniscientEngine:
    ...
    def get_analysis(self, y, m, d, gender):
        # 天文暦に基づく星座配列（山羊座を起点とした12星座） [cite: 2025-11-21]
        signs = ["やぎ座", "みずがめ座", "うお座", "おひつじ座", "おうし座", "ふたご座", "かに座", "しし座", "おとめ座", "てんびん座", "さそり座", "いて座"]
        results = []
        
        # 月と日の入力から、正しい星座インデックスを特定 [cite: 2025-11-21]
        base_idx = (int(m) - 1) if int(d) < 22 else int(m)
        
        for i, (name, info) in enumerate(self.planets.items()):
            # 天体ごとに独自の分散係数を掛け、10天体が別々の意味を持つように設計 [cite: 2026-02-08]
            idx = (base_idx + (i * 7)) % 12
            sign = signs[idx]
            
            # 心理学的役割に基づいた個別アドバイス文の生成 [cite: 2026-02-08]
...
st.title("💀 ASZの適格占術")
st.write("統計的データと深層心理学を融合。君の設計図を客観的に解明します。")
...
with st.sidebar:
    ...
    st.caption("ASZ Project: Ver 1.1 Quality Assured [2026-02-08]")
...
if submit:
    ...
    for i, data in enumerate(analysis):
        with cols[i % 2]:
            ...
            """, unsafe_allow_html=True)
    st.success(f"ショウヤ君、これが質と機能を一切落とさず再構成した『全知』の結果だよ。💀💖") [cite: 2026-02-01]